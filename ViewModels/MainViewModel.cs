...
namespace ShifterUser.ViewModels
{
    public class MainViewModel
    {
        ...
        {  // 메인 프레임 설정(예: MainWindow.xaml.cs에서 호출)
            ...
            Navigate(PageType.Page);        // 첫 페이지 로딩 ( Enum PageType.Start )
        }
        ...
        private void Navigate(PageType page)
        {
            ...
            switch (page)
            {
                ...
                case PageType.NoticeDetail:
                    _mainFrame.Navigate(new NoticeDetailView { DataContext = App.Services.GetRequiredService(typeof(NoticeDetailViewModel)) });
                    break;

...
namespace Shifter.Services {
    ...
    public interface ISocketManager {
        ...
        Task SendAsync(WorkItem item);
        Task<WorkItem> ReceiveAsync();
        void Close();
    }
    ...
    public class SocketManager : ISocketManager {
        ...
        public async Task SendAsync(WorkItem item) {
            ...
            if( stream is not null ) {
                ...
                await stream!.WriteAsync(jsonBytes, 0, jsonLen);
                if (payloadLen > 0)
        ...
        public async Task<WorkItem> ReceiveAsync() {
            Console.WriteLine("\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
            Console.WriteLine("[SocketManager] Executed public WorkItem Receive()");

            byte[] header = await ReadExact(8); // 헤더 8바이트 읽기
            ...
            // JSON 부분 읽기
            byte[] jsonBytes = await ReadExact(jsonLen);
            string json = Encoding.UTF8.GetString(jsonBytes);
            Console.WriteLine("Read json");

            // 파일 부분 (있다면) 읽기
            byte[] payload = payloadLen > 0 ? await ReadExact(payloadLen) : new byte[0];
        ...
        private async Task<byte[]> ReadExact(int size) {
            ...
            while (totalRead < size) {
                int read = 0;
                if (stream != null)
                    read = await stream.ReadAsync(buffer, totalRead, size - totalRead);
                if (read == 0)
